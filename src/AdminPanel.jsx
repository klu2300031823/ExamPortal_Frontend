...
export default function AdminPanel({ token }) {
  ...
  const fetchQuestions = async () => {
    try {
      let res = await axios.get("http://localhost:30083/exam-backend/api/exam/questions", {
  ...
  const fetchResults = async () => {
    try {
      let res = await axios.get("http://localhost:30083/exam-backend/api/exam/results", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setResults(res.data);
    } catch (e) {
  ...
  const saveQuestion = async () => {
    try {
      if (editingId) {
        await axios.put(`http://localhost:30083/exam-backend/api/exam/update/${editingId}`, q, {
          headers: { Authorization: `Bearer ${token}` },
        });
        alert("✅ Question updated");
        setEditingId(null);
      } else {
        await axios.post("http://localhost:30083/exam-backend/api/exam/add", q, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const deleteQuestion = async (id) => {
    try {
      await axios.delete(`http://localhost:30083/exam-backend/api/exam/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
